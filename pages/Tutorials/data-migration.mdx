...
The `ProcessorDataset` class allows `bench-kit` to process your data, zip it, and upload it to the cloud.
...
Great! We now have a class to upload our data, and a class to download the data. Now bench-kit has the information it
...
To train your model in pytorch a dataset is wrapped with a dataloader. This process does not change with `bench-kit`,
however we recommend you make a call to get_dataloader. `get_dataloader` will return you the dataloader wrapped around
your ChunkDataset synced for the cloud without any other manual effort. To get your dataloader simply pass in your
ChunkDataset and the name of the dataset. Optionally you can pass in the worker count and the batch size.
